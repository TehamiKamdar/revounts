...
class NEXStats {
	...
	function __construct($url="google.com",$timeout=5) {
	...
	public function getDmozListing() {
		...
		$dmoz_result = strtolower($this->getRemoteContents('http://www.dmoz.org/search/?q=' . urlencode($this->url)));
	...
	public function getSpeedScore($key) {
	
		$userIP = $this->getClientIP();
		
		$contents = $this->getRemoteContents('https://www.googleapis.com/pagespeedonline/v1/runPagespeed?fields=score&key='.$key.'&strategy=desktop&url=http://' .$this->url);  
	...
	public function getBacklinks($accessID,$secretKey) {
		...
		$backLinks = $this->getStringBetween($content,'{"uid":','}');
		
		if(is_numeric($backLinks))
			return $backLinks;
	}
	...
	public function getFacebookStats() {
		...
		return json_decode($this->getRemoteContents("http://api.facebook.com/restserver.php?method=links.getStats&urls=http://$this->url&format=json"),true);
